...
class ImageProc(threading.Thread):
    def __init__(self):
        ...
        self.latestImg = []
        self.feedback = []
        self.thresholds = {'low_red':140,'high_red':255,'low_green':0,'high_green':255,'low_blue':55,'high_blue':100}
    ...
    def doImgProc(self):
        ...
        theMask = cv2.inRange(hsvImg, low, high)
        ...
        center = (stats[index][2],stats[index][3])
        circleColor = (0,0,0) 

        cv2.circle(theMask, center,int((max/(2*numpy.pi))), circleColor, 2)
